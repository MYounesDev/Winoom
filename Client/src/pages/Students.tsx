...
const Students = () => {
  ...
  const getSortIndicator = (field: SortField) => {
    ...
    return (
      <ArrowUpDown 
        className={`inline ml-1 h-4 w-4 ${sortDirection === "asc" ? "text-blue-500" : "text-red-500"}`} 
      />
    );
  };
  ...
  const generateUniqueID = (): number => {
    ...
    // Sort existing IDs
    const existingIDs = students.map(student => student.ID).sort((a, b) => a - b);
    ...
    <PageTemplate title="Student Management">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md transform transition-all animate-fadeIn border border-gray-200">
            ...
            <form onSubmit={handleSubmitStudent} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm"
              ...
              <div>
                ...
                >
                  ...
                  <option value="Suspended" className="bg-red-100 text-red-800">Suspended</option>
                  <option value="Graduated" className="bg-blue-100 text-blue-800">Graduated</option>
                </select>
                {/* Color indicator for selected status */}
                <div className="mt-2 px-3 py-1 inline-block rounded-full text-sm font-medium text-white" 
                  style={{ 
                    backgroundColor: 
                      newStudent.status === "Active" ? "#10B981" : 
                      newStudent.status === "Inactive" ? "#6B7280" : 
                      newStudent.status === "Suspended" ? "#EF4444" : 
                      "#3B82F6" 
                  }}
                >
                  {newStudent.status}
              ...
              <div className="pt-4 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors font-medium shadow-sm"
                  disabled={isSubmitting}
                >
                  {isSubmitting ? (
